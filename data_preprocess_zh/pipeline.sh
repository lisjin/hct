...
for split in 'train' 'dev' 'test'; do
  ...
  # Lemmas: rewrite/<split>/unfound_lems.json
  # Parse trees: rewrite/<split>/cpt*.txt
  python proc_unmatch.py --split ${split} --lem --cparse --data_dir $DATA_DIR

  python rule_extract.py --split ${split} --data_dir $DATA_DIR --max_sp_width 2 --min_rule_prop .005

  # rewrite_out/<split>/{sentences|tags}.txt
  python rw_unfound.py --split ${split} --vocab_f $VOCAB_F --write --data_dir $DATA_DIR --data_out_dir ${DATA_DIR}_out
done
